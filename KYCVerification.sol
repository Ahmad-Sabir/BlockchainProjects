...
contract kycVerification {
    ...
    function addNewBankbyRBI(string memory _bankName, address _bankAddress)
        public
        onlyOwner
    {
    ...
    function blockBankFromAddingCustomer(address _bankAddress)
        public
        onlyOwner
    {
    ...
    function kycStatus(address _bankAddress)
        public
        view
        returns (string memory)
    {
        if (banks[_bankAddress].blocked == false) {
            if (banks[_bankAddress].kycPermission == true) {
                return (
                    "Banks are Allowed to Add Customer and Allowed to do KYC"
                );
            } else {
                return (
                    "Banks are Allowed to Add Customer but Not Allowed to do KYC"
                );
            }
        } else {
    ...
    function addNewCustomerbyBank(
        string memory _bankName,
        address _bankAddress,
        uint32 _customerID,
        uint64 _aadhaarNumber,
        string memory _customerName
    ) public {
        require(
            banks[_bankAddress].blocked == false,
            "You do not Have the Permission to Add Customer"
        );
    ...
    function queryCustomerByID(uint64 _customerID)
        public
        view
        returns (
            string memory _customerName,
